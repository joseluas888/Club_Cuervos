...
struct PaginaPrincipalView: View {
    ...
    @State var g_nombre:String = "Nombre"
    @State var g_folio:Int = 0
    @State var g_cantidad:String = "000"

    var body: some View {
        VStack {
            TabView {
                RecibosPorCobrar(fichas_porCobrar: self.$fichas_porCobrar, fichas_cobradas: self.$fichas_cobradas, g_nombre: self.$g_nombre, g_folio: self.$g_folio, g_cantidad: self.$g_cantidad)
                    .badge(fichas_porCobrar.count)
                    .tabItem { Label("Por cobrar", systemImage: "dollarsign.circle.fill") }
                RecibosYaCobradosView(fichas_cobradas: $fichas_cobradas)
